...
tuple* read_record_from_file(FILE *pfile, int index)
{
    ...
    tuple* record = malloc(sizeof(tuple));
    fread(record, sizeof(tuple), 1, pfile);
...
void write_record_in_file(FILE *pfile, const tuple *record_to_write, int index)
{
    ...
    fwrite(record_to_write, sizeof(tuple), 1, pfile);
...
{
    return get_file_size_in_bytes(pfile) / sizeof(tuple);
}
